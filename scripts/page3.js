...
function helloApi(num1Input,num2Input){
    var letters = /^[A-Za-z]+$/;
    var numbers = /^[0-9amp]+$/;
    if(num1Input.value.match(letters)&&num2Input.value.match(numbers))
    {
        savedInput = num1Input.value;
        savedInput2 = num2Input.value;
        sayHelloUrl = "https://allforonewebapijeremy1.azurewebsites.net/askingquestions/asking/" + savedInput + '/' + savedInput2;
        urlCall(sayHelloUrl);
    }
    else{
